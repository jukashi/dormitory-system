...
?>
  <section class="dashboard-hero">
    ...
    <div class="dashboard-hero-actions"><?php if (has_permission('tenants')): ?><a class="button-link" href="tenants.php?action=add">+ Add tenant</a><?php endif; ?><?php if (has_permission('calendar')): ?><a class="hero-link" href="calendar.php?shift=ADMIN">View Admin Calendar</a><?php endif; ?></div>
  </section>
  <section class="stats-grid dashboard-stats">
    <article class="stat-card stat-tenants"><span class="stat-icon">⌂</span><div><span>Active tenants</span><strong><?= $activeTenants ?></strong><?php if (has_permission('tenants')): ?><a href="tenants.php">View tenant list <b>→</b></a><?php endif; ?></div></article>
    <article class="stat-card stat-payments"><span class="stat-icon">$</span><div><span>Collected this month</span><strong>NT$ <?= number_format($collected, 2) ?></strong><?php if (has_permission('payments')): ?><a href="payments.php">View payments <b>→</b></a><?php endif; ?></div></article>
    <article class="stat-card stat-occupancy"><span class="stat-icon">▦</span><div><span>Bed occupancy</span><strong><?= $occupiedBeds ?> <small>/ <?= $totalBeds ?></small></strong><?php if (has_permission('dormitories')): ?><a href="dormitories.php">Manage rooms <b>→</b></a><?php endif; ?></div></article>
    <article class="stat-card stat-maintenance"><span class="stat-icon">⚒</span><div><span>Open maintenance</span><strong><?= $openMaintenance ?></strong><?php if (has_permission('maintenance')): ?><a href="maintenance.php?filter=open">View requests <b>→</b></a><?php endif; ?></div></article>
  </section>
  <div class="management-grid dashboard-activity-grid">
    <section class="panel dashboard-panel"><div class="dashboard-panel-title"><div><p class="eyebrow">Finance</p><h2>Recent payments</h2></div><?php if (has_permission('payments')): ?><a href="payments.php">All payments →</a><?php endif; ?></div><?php if (!$recentPayments): ?><div class="empty-state"><span>◌</span><p>No payments recorded yet.</p></div><?php else: ?><div class="table-panel"><table><thead><tr><th>Tenant</th><th>Amount</th><th>Date</th></tr></thead><tbody><?php foreach ($recentPayments as $payment): ?><tr><td><?= e($payment['full_name']) ?></td><td><strong>NT$ <?= number_format((float) $payment['amount'], 2) ?></strong></td><td><?= e($payment['payment_date']) ?></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?></section>
    <section class="panel dashboard-panel"><div class="dashboard-panel-title"><div><p class="eyebrow">Schedule</p><h2>Upcoming events</h2></div><?php if (has_permission('calendar')): ?><a href="calendar.php?shift=ADMIN">Admin Calendar →</a><?php endif; ?></div><?php if (!$upcomingEvents): ?><div class="empty-state"><span>◌</span><p>No upcoming schedule events.</p></div><?php else: ?><div class="table-panel"><table><thead><tr><th>Person</th><th>Event</th><th>Date</th></tr></thead><tbody><?php foreach ($upcomingEvents as $event): ?><tr><td><?= e($event['full_name']) ?></td><td><span class="event-pill"><?= e(ucwords(str_replace('_', ' ', $event['event_type']))) ?></span></td><td><?= e($event['start_date']) ?></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?></section>
  </div>
