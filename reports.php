...
if($action==='export'){
 ...
 if($type==='outstanding'){$s=$pdo->prepare("SELECT t.full_name,t.monthly_rent,t.contact_no,t.employee_id FROM tenants t WHERE t.status='active' AND NOT EXISTS (SELECT 1 FROM payments p WHERE p.tenant_id=t.id AND p.payment_month=:month) ORDER BY t.full_name");$s->execute(['month'=>$month.'-01']);$rows=[];foreach($s as $r)$rows[]=[$r['full_name'],$r['monthly_rent'],$r['contact_no'],$r['employee_id']];csv_out('outstanding-'.$month.'.csv',['Tenant','Monthly Rent (NT$)','Contact','Employee ID'],$rows);}
...
$out=$pdo->prepare("SELECT t.full_name,t.monthly_rent,t.contact_no FROM tenants t WHERE t.status='active' AND NOT EXISTS(SELECT 1 FROM payments p WHERE p.tenant_id=t.id AND p.payment_month=:month) ORDER BY t.full_name");
...
$outstandingRent=array_sum(array_column($outstanding,'monthly_rent'));
...
<section class="panel table-panel reports-panel">
  ...
  <table><thead><tr><th>Tenant</th><th>Monthly rent</th><th>Contact</th></tr></thead><tbody><?php if(!$outstanding):?><tr><td colspan="3"><div class="report-empty success">All active tenants have a payment for this month.</div></td></tr><?php else:foreach($outstanding as $row):?><tr><td><strong class="primary-cell"><?=e($row['full_name'])?></strong></td><td><span class="rent-due">NT$ <?=number_format((float)$row['monthly_rent'],2)?></span></td><td><?=e($row['contact_no'])?></td></tr><?php endforeach;endif;?></tbody></table>
</section>
